...
from SeqCondGenVariants import SeqCondGenX # stochastic attention
...
if 1:
    ...
    if dataset == 'mnist':
        ...
        datasets = load_udm(datapath, as_shared=False, zero_mean=False)
...
def test_seq_cond_gen_sequence(step_type='add', dataset='mnist'):
    ##############################
    # File tag, for output stuff #
    ##############################
...
def test_seq_cond_gen_bouncing_balls(step_type='add'):
    ...
    SCG = SeqCondGen(
                x_and_y_are_seqs=1,#False, # this test doesn't use sequential x/y
                total_steps=total_steps,
                init_steps=init_steps,
                exit_rate=exit_rate,
                nll_weight=0.0, # ignored, because x_and_y_are_seqs == False
    ...
    try:
        ...
        pass#print "visualize_attention failed"
